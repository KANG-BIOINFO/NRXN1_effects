...
setwd('/data/aronow/Kang/induced_neuron/ChangHui_RNAseq_fastq/isoform_RSEM/rsem_transcripts/differential_v2_DEG/')
...
isoform_files = paste0('../transcripts/', samples, '.genes.results')
names(isoform_files) = samples
txi <- tximport(files = isoform_files, type = "rsem", txIn = F, txOut = F)
...
dds <- DESeq(dds)
res = results(dds)
rownames(res_table) = rownames(res_table)
saveRDS(dds, 'DESeq2_object_donor.rds')
saveRDS(res, 'DESeq2_result_donor.rds')
# write.xlsx(as.data.frame(res), 'DE_isoforms_donor.xlsx', row.names=T)
...
dds <- DESeqDataSetFromTximport(txi, colData = df_eng, design = ~Pair.Batch + NRXN1.genome)
dds <- DESeq(dds)
res = results(dds)
saveRDS(dds, 'DESeq2_object_engineered.rds')
saveRDS(res, 'DESeq2_result_engineered.rds')
# write.xlsx(as.data.frame(res), 'DE_isoforms_engineered.xlsx')
